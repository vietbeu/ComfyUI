...
@torch.no_grad()
def sample_euler_ancestral(model, x, sigmas, extra_args=None, callback=None, disable=None, eta=1., s_noise=1., noise_sampler=None, end_cfg=0.6):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        print(f"i: {i}/{len(sigmas) - 1}, extra_args: {extra_args}, model: {model.inner_model}")
        if end_cfg is not None and i / (len(sigmas)-1) > end_cfg:
            model.inner_model.set_cfg(1)
